...
# FETCH CAPA OUTPUT
# ==========================================================
def fetch_capa_output(bucket: str, key: str) -> dict:
    """Fetch CAPA Agent output JSON from S3."""
    s3 = get_s3_client()
...
# SEVERITY MAPPING
# ==========================================================
def map_severity(z_category: str) -> str:
    z = z_category.lower()
    if "large" in z:
        return "High"
    elif "moderate" in z:
...
# ==========================================================
def evaluate_compliance(sensor: dict) -> str:
...
# ==========================================================
def call_bedrock(prompt_text: str) -> dict:
...
# ==========================================================
async def process_sensor(sensor: dict, semaphore: asyncio.Semaphore, executor: ThreadPoolExecutor) -> dict:
...
"""
        ...
        try:
            ...
            result = {
                "failure_reason": "Error generating summary",
...
# MAIN ASYNC ENGINE
# ==========================================================
async def run_compliance_engine(bucket: str, key: str) -> list:
    """
    Run compliance evaluation on CAPA output stored in S3.

    Args:
        bucket: S3 bucket containing the CAPA Agent output
        key: S3 key of the CAPA output JSON file

    Returns:
        List of row-level compliance results
    """
    print(f"Fetching CAPA data from s3://{bucket}/{key}")
    data = fetch_capa_output(bucket, key)
...
# UPLOAD RESULT TO S3
# ==========================================================
def upload_output(result: list, bucket: str) -> str:
    """
    Upload compliance results to S3.

    Args:
        result: List of row-level compliance results
        bucket: S3 bucket to upload results to

    Returns:
        S3 URI of the uploaded compliance output
    """
    s3 = get_s3_client()
    ...
    print(f"Successfully uploaded to s3://{bucket}/{output_key}")
